...
import { download as downloadLibMpv } from "./libmpv.ts";
...
export async function download(options: PerpareOptions): Promise<string> {
  ...
  const denoPluginsPath = path.resolve(".deno_plugins");
  const localPath = path.join(denoPluginsPath, cacheFileName);
  ...
  if (!(await exists(localPath)) || !checkCache) {
    ...
    if (remoteUrl.startsWith("file://")) {
      ...
      if (os === "darwin") {
        await downloadLibMpv(denoPluginsPath);
      }
    }
  }
